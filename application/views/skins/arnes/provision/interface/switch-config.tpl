...
<p>
Please apply the following configuration to: <code>{$progress.Physicalinterface.Switchport.SwitchTable.name}.arnes.si</code>
</p>
...
{elseif $progress.Physicalinterface.Switchport.SwitchTable.model eq 'WS-C4900M'}

<pre>
conf t
interface {$progress.Physicalinterface.Switchport.name}
 description -- SIX - {$progress.Cust.shortname} (AS{$progress.Cust.autsys}) --
 switchport
 switchport access vlan {$progress.Vlaninterface.Vlan.number}
 switchport mode access
 switchport port-security
 logging event link-status
