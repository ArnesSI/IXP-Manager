...
<ul>
    <li>
        Where a SIX member is not listed on this peering matrix, it is because they are
        currently not actively peering at SIX, or because they have opted out of presenting
        their peering information in this database.
    </li>
    <li>
        This peering matrix is based on Netflow traffic accounting data from the SIX peering
        LANs. It is significantly more accurate the the old RIPE IRRDB peering matrix, which
        is still actively maintained by SIX.
    </li>
    <li>
        This peering matrix only detects if there is bidirectional TCP flow between routers at
        SIX. It cannot detect whether there are actually prefixes swapped betwen routers.
    </li>
    <li>
